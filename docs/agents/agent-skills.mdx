...
- **Project**: `.mux/skills/<skill-name>/SKILL.md`
...
For a workspace that executes in a subproject, Xum checks the working directory and each parent directory through the checkout root. It never searches above the checkout. A nearer project definition wins over a farther one; within the same directory, `.mux/skills` wins over `.agents/skills`.
...
Global plugins can be installed from git via **Settings → Plugins** (paste a git URL or `owner/repo[@ref]`); the install preview lists every skill the plugin would contribute before anything is written.
...
By default, Xum _advertises_ skills by listing them in the system prompt’s `<agent-skills>` index.
