...
MCP (Model Context Protocol) servers provide additional tools to agents. Configure them globally in `~/.xum/mcp.jsonc`, with optional repo overrides in `./.mux/mcp.jsonc`.
...
Or directly in your MCP config file (global: `~/.xum/mcp.jsonc`, repo overrides: `./.mux/mcp.jsonc`):
...
- **Configuration** is global with optional repo overrides — Global config lives in `~/.xum/mcp.jsonc`, with optional repo overrides in `./.mux/mcp.jsonc`
...
Xum supports per-workspace MCP overrides (enable/disable servers and restrict tool allowlists) without modifying the shared config files (`~/.xum/mcp.jsonc` / `./.mux/mcp.jsonc`).
...
- Older Mux versions stored these overrides in `~/.mux/config.json`; Xum will migrate them into `.mux/mcp.local.jsonc` on first use
