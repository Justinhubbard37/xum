...
export const BUILTIN_SKILL_FILES: Record<string, Record<string, string>> = {
  ...
  loop: {
    "SKILL.md": [
      ...
      "- **Does not depend on the result:** use `run_in_background: true`. Xum treats background work as non-blocking (internal `notify_on_terminal` policy) and wakes the workspace later with the result; you do not need to force-await it just to end the turn.",
      "",
  ...
  spawn: {
    "SKILL.md": [
      ...
      "",
      "Default to waiting for the sub-agent, then integrate and report its result. If the user explicitly asks to start the work in the background or be notified later, use `run_in_background: true`, report the task ID, and end the turn; Xum will wake the workspace when the sub-agent reaches a terminal state so you can integrate the result then. Do not background `/spawn` when the current answer depends on the result.",
      "",
    ].join("\n"),
  },
  "workflow-authoring": {
  ...
  "xum-docs": {
    "references/docs/adr/0003-context-boundaries-for-compaction-and-reset.md": [
      ...
      "## Context",
      "",
      "Xum needs two ways to start a new active conversation context without treating all persisted transcript history the same way. Compaction summarizes earlier history into provider-visible content, while `/clear --soft` should preserve earlier transcript history but keep it out of future provider requests.",
      "",
      "## Decision",
      "",
      "Xum will model both compaction and context reset as kinds of Context Boundary. A Compaction Boundary carries a provider-visible summary of earlier transcript history. A Context Reset Boundary is visible transcript structure but is provider-invisible: it preserves earlier Transcript History while starting a new Active Conversation Context.",
      "",
    ...
    "references/docs/adr/0004-cli-goal-runs-are-not-strict-goal-aliases.md": [
      ...
      "description: Architecture decision for giving xum run --goal CLI-specific completion and limit semantics",
      "---",
      "",
      ...
      "## Context",
      "",
      "`xum run` is designed for automation: it normally sends one request, streams the result, and exits. Interactive `/goal` is a workspace lifecycle command with defaults, controls, and cooldown behavior that assume a user can intervene from the UI.",
      "",
      "Adding `xum run --goal` creates a different automation need. A script needs one process to keep driving an objective until there is an authoritative completion signal, while still preserving goal accounting and model-facing goal tools.",
      "",
      "## Decision",
      "",
      "Xum will model `xum run --goal` as a CLI Goal Run, not as a strict alias for interactive `/goal`.",
      "",
      "A CLI Goal Run creates an ephemeral goal for the `xum run` process, sends either the provided message/stdin or the goal text as the kickoff message, and continues in exec mode until the persisted goal status is `complete` or a stop condition is reached. Interactive goal defaults are not applied; omitted `--goal-budget` and `--goal-turns` mean no goal-specific limit. The existing session `--budget` remains a separate hard stop.",
      "",
    ...
    "references/docs/agents/index.mdx": [
      ...
      "## Overview",
      "",
      "An **agent** in Xum owns two things for a given turn:",
      "",
      ...
      "## Quick Start",
      "",
      "Drop a Markdown file in `.xum/agents/` (project) or `~/.xum/agents/` (global):",
      "",
      "```md",
      "---",
      "name: Review",
      ...
      "",
      "Xum scans three roots, **non-recursive** (only direct `.md` children):",
      "",
      "| Location             | Scope   | Priority |",
      "| -------------------- | ------- | -------- |",
      "| `.xum/agents/*.md`   | Project | Highest  |",
      "| `~/.xum/agents/*.md` | Global  | Medium   |",
      ...
      "# skip_init_hook only skips the .xum/init hook; runtime provisioning",
      ...
      "### Markdown body",
      "",
      "The body becomes the agent's instruction prompt, layered with Xum's base prelude, the workspace environment context, and `AGENTS.md`.",
      "",
      ...
      "",
      "- **Same name as the current file** — Xum skips the current file's scope, so a project `exec.md` with `base: exec` resolves to the global or built-in `exec` (not itself).",
      "- **Different name** — Xum anchors the lookup at the current scope or lower. A built-in's `base: foo` cannot pull in a project-level `foo.md`.",
      "",
      ...
      "",
      "Each agent has one defaults entry (`agentAiDefaults`), configured in **Settings → Agents**:",
      "",
      "- **Base fields** (`modelString`, `thinkingLevel`, `reasoningMode`) apply when you select the agent in the workspace.",
      "- **Delegated overrides** (the sparse nested `subagent` profile) apply only when the agent is spawned via `task`, and win over the base fields per field.",
      "",
      "Delegated overrides inherit per field from the base fields; base fields inherit per field from the agent's frontmatter `ai` block. You can override one field (e.g. only `thinkingLevel`) and leave the others inherited.",
      "",
    ...
    "references/docs/agents/instruction-files.mdx": [
      ...
      "  to the agent. Move any `## Model: ...` sections into `.xum/AGENTS.md` (or `~/.xum/AGENTS.md`). See",
      "  [Model Prompts](#model-prompts) for details.",
      "</Warning>",
      "",
      "## Overview",
      "",
      "Xum layers instructions from multiple locations. At each location, Xum loads instructions from two",
      "kinds of files:",
      "",
      ...
      "3. `CLAUDE.md`",
      "",
      "Xum picks the first match. If found, it also appends `AGENTS.local.md` from the same directory",
      "(when present).",
      "",
      "**Xum-dedicated instruction files** — loaded alongside the shared file:",
      "",
      "- `<dir>/.xum/AGENTS.md` (+ optional `<dir>/.xum/AGENTS.local.md`)",
      "",
      "Only the exact name `AGENTS.md` is supported inside `.xum/` — no `AGENT.md` or `CLAUDE.md`",
      "fallback. A directory may have only `.xum/AGENTS.md` and no shared file.",
      "",
      "### Instruction locations",
      "",
      "Instructions are collected from (most general to most specific):",
      "",
      "1. `~/.claude/CLAUDE.md` when the **Claude compatibility** experiment is enabled",
      "2. `~/.xum/AGENTS.md` as native global defaults (Xum-dedicated by construction)",
      "3. Workspace root with its shared file and `<workspace>/.xum/AGENTS.md`",
      "4. Sub-project and each repo directory in multi-project workspaces with their shared file and `.xum/AGENTS.md`",
      "",
      "The Claude compatibility source has the lowest global precedence, so native `~/.xum` instructions always layer on top. Xum reads only the exact `~/.claude/CLAUDE.md` file: it does not probe `AGENTS.md`, `AGENT.md`, local variants, or a `.xum` subdirectory there, and it never writes to `~/.claude`. This file uses shared-file semantics, so `Model:` and `Mode:` headings remain ordinary markdown while `Tool:` sections still work.",
      "",
      "<Info>",
      "  Xum strips HTML-style markdown comments (`<!-- ... -->`) from instruction files before sending them",
      "  to the model. Use these comments for editor-only metadata—they will not reach the agent.",
      "</Info>",
      "",
      "## Scoped instructions",
      "",
      "Xum supports **scoped instructions** that activate only in specific contexts. You define them using",
      "special headings:",
      "",
      "| Directive        | Where it works           | Effect                              |",
      "| ---------------- | ------------------------ | ----------------------------------- |",
      "| `Model: <regex>` | Xum-dedicated files only | Active for specific models          |",
      "| `Mode: <mode>`   | Xum-dedicated files only | Active for specific modes or agents |",
      "| `Tool: <name>`   | All instruction sources  | Appended to a tool's description    |",
      "",
      "**Xum-dedicated sources** (checked in this priority order):",
      "",
      "1. Agent definition body (`.xum/agents/<name>.md`)",
      "2. Workspace/project `.xum/AGENTS.md` files",
      "3. Global `~/.xum/AGENTS.md`",
      "",
      ...
      "  customize behavior per agent. `Mode:` sections are available, but only in Xum-dedicated files (see",
      ...
      "  workspace `.xum/AGENTS.md` → global `~/.xum/AGENTS.md`.",
      "- **Concatenation**: For `Model:` and `Mode:` sections, **all** matching sections across",
      '  Xum-dedicated sources are concatenated and injected — it is not "first match wins."',
      ...
      "  Move `## Model: ...` sections to `.xum/AGENTS.md` (or `~/.xum/AGENTS.md`).",
      "</Warning>",
      "",
      "`Model:` is honored only in Xum-dedicated sources: agent definitions, `.xum/AGENTS.md` files, and",
      "`~/.xum/AGENTS.md`.",
      "",
      "**Syntax**: `Model: <regex>`",
      "",
      ...
      "",
      "**Example** (in `.xum/AGENTS.md` or `~/.xum/AGENTS.md`):",
      "",
      "```markdown",
      "## Model: sonnet",
      "",
      ...
      "`<mode-...>` tag.",
      "",
      "`Mode:` is honored only in Xum-dedicated sources: agent definitions, `.xum/AGENTS.md` files, and",
      "`~/.xum/AGENTS.md`.",
      "",
      "**Syntax**: `Mode: <mode>`",
      "",
      ...
      "`.xum/AGENTS.md`, and agent definitions.",
      "",
      ...
      "```",
      "~/.claude/",
      "  CLAUDE.md          # Optional read-only compatibility source",
      "",
      "~/.xum/",
      "  AGENTS.md          # Native global instructions (Xum-dedicated)",
      ...
      "  AGENTS.local.md    # Personal tweaks (gitignored)",
      "  .xum/",
      "    AGENTS.md        # Xum-only instructions (Model:/Mode: directives go here)",
      "    AGENTS.local.md  # Personal Xum-only tweaks (gitignored)",
      "```",
      "",
    ].join("\n"),
    "references/docs/agents/plan-mode.mdx": [
      ...
      "",
      "```",
      "~/.xum/plans/<project>/<workspace-name>.md",
      "```",
      "",
      "Notes:",
      "",
      ...
      "",
      "```bash",
      'xum run --mode plan "Design a caching strategy for the API"',
      "```",
      "",
    ...
    "references/docs/agents/prompting-tips.mdx": [
      ...
      "```",
      "",
      "Xum will run compaction and then automatically send your follow-up message.",
      "",
    ...
    "references/docs/config/agentic-git-identity.mdx": [
      ...
      "---",
      "",
      "Configure Xum to use a separate Git identity for AI-generated commits, making it easy to distinguish between human and AI contributions. Reasons to use a separate identity include:",
      "",
      "- Clear attribution",
      ...
      '   - **Note**: "Xum agent token" (or similar)',
      ...
      "",
      "Add the Git identity environment variables as [Project Secrets](/config/project-secrets) in Xum:",
      "",
      "1. Open Xum and find your project in the sidebar",
      ...
      "",
      "<Info>",
      "  If you need the agent identity outside of Xum, you can alternatively set these as global",
      ...
      "### Using a Git Hook",
      "",
      "Create a `prepare-commit-msg` hook that adds co-author attribution only when running inside Xum:",
      "",
      "```bash",
      "#!/bin/bash",
      ...
      "COMMIT_SOURCE=$2",
      "",
      "# Only add co-author when running in Xum",
      'if [ -z "$XUM_RUNTIME" ]; then',
      "  exit 0",
      "fi",
      "",
      "# Skip merge commits",
      ...
      "  involvement. The hook only activates inside Xum workspaces (detected via `XUM_RUNTIME`).",
      "</Info>",
      "",
      "### Comparison",
      "",
    ...
    "references/docs/config/keybinds.mdx": [
      ...
      "description: Complete keyboard shortcut reference for Xum",
      "---",
      "",
      "Xum is designed to be keyboard-driven for maximum efficiency. All major actions have keyboard shortcuts.",
      "",
      "<Info>",
    ...
    "references/docs/config/notifications.mdx": [
      ...
      "## Overview",
      "",
      "Xum can send system notifications to alert you about important events. Notifications appear as native OS notifications (macOS Notification Center, Windows Toast, Linux notification daemon) and work even when Xum is in the background. Clicking a notification takes you directly to the workspace that sent it.",
      "",
      ...
      "",
      "Notification settings (sound, Do Not Disturb, etc.) are controlled by your OS preferences, not Xum.",
      "",
      "## Tool definition",
      "",
    ...
    "references/docs/config/project-secrets.mdx": [
      ...
      "---",
      "",
      "Securely manage environment variables for your projects in Xum. Project secrets are automatically injected when the agent executes bash commands, making it easy to provide API keys, tokens, and other sensitive configuration.",
      "",
      ...
      "- **Stored outside repo** in `~/.xum/secrets.json`",
      ...
      "### Storage",
      "",
      "- Secrets are stored in `~/.xum/secrets.json` (outside your repo)",
    ...
    "references/docs/config/providers.mdx": [
      ...
      "| `baseUrl`      | Yes      | The OpenAI-compatible API base URL. Xum appends `/v1` when the URL has no path; a trailing slash (`http://host:8080/`) keeps requests at the origin root. |",
      ...
      "",
      "Custom providers always use Chat Completions. If the configured `baseUrl` has no path, such as",
      "`http://localhost:8080`, Xum appends `/v1`; explicit paths remain unchanged. For servers that",
      "serve the API at the origin root, add a trailing slash (`http://localhost:8080/`) to skip the",
      "`/v1` suffix. To configure a",
      "single gateway through the built-in `openai` provider instead, see [OpenAI wire format](#openai-wire-format).",
      "",
      ...
      "```",
      "",
      "Login is available from the desktop app or a locally hosted Xum server. On a remote Xum",
    ...
    "references/docs/config/vim-mode.mdx": [
      "---",
      "title: Vim Mode",
      "description: Vim-style editing in the Xum chat input",
      "---",
      "",
      "Xum includes a built-in Vim mode for the chat input, providing familiar Vim-style editing for power users.",
      "",
      "## Enabling Vim Mode",
      "",
    ...
    "references/docs/docs.json": [
      ...
      '    { "source": "/workspaces/muxignore", "destination": "/workspaces/xumignore" },',
    ...
    "references/docs/getting-started/mux-gateway.mdx": [
      "---",
      "title: Xum Gateway",
      "description: Log in to Xum Gateway to get evaluation credits",
      "---",
      "",
      "Xum Gateway is an optional provider that proxies requests through Coder's",
      ...
      "your credit balance.",
      "",
      "Xum Gateway currently doesn't provide any utility other than allowing subsidized",
      ...
      "",
      "If you're not eligible (or you run out of credits), you can still use Xum by",
      "configuring providers directly with your own API keys.",
      "",
      "## Xum Voucher Codes",
      "",
      "If you receive a **Xum Voucher Code** (for example, at an event), redeem it on",
      ...
      "3. In Xum, click **Login to Xum Gateway** and sign in with that same GitHub",
      "   account — your updated balance should show up in Settings.",
      "",
      "## Log in",
      "",
      "1. Open Xum (see [install](/install) for instructions).",
      ...
      "",
      "1. In **Settings → Models**, turn on **Xum Gateway** for the model(s) you want",
      "   to use.",
      "2. Optionally, click **Star** to set a model as your default model.",
      "",
      "## Our Ask",
      "",
      "As Xum is in its early development stage, we highly value user feedback. Please",
    ...
    "references/docs/getting-started/why-parallelize.mdx": [
      ...
      "  - Xum shows an indicator when the model finishes.",
    ...
    "references/docs/guides/github-actions.mdx": [
      ...
      "description: Automate your workflows with xum run in GitHub Actions",
      "---",
      "",
      "`xum run` is designed for automation scenarios like CI/CD pipelines. This guide shows how to integrate Xum into your GitHub Actions workflows.",
      "",
      "## Prerequisites",
      "",
      ...
      "2. **npm/bun**: The workflow will install Xum via `bunx` or `npx`",
      "",
      "## Basic Usage",
      "",
      "Here's a minimal example that runs Xum in a GitHub Action:",
      "",
      "```yaml",
      "- name: Xum Review",
      "  env:",
      ...
      '  run: bunx @coder/xum run "Review this PR for critical issues. If any are found, exit with 1 to block merge."',
      "```",
      "",
      "`xum run` supports agent-controlled exit codes, so the workflow step can fail when issues are found.",
      "",
      "### Key Options for CI",
      "",
      "| Option           | Purpose                                        |",
      ...
      "",
      "This is the exact workflow used live in the Xum repo (see [`.github/workflows/auto-cleanup.yml`](https://github.com/coder/mux/blob/main/.github/workflows/auto-cleanup.yml)). It runs periodically to identify low-risk cleanup opportunities and maintains a refactor PR with improvements.",
      "",
      ...
      "",
      "Xum's repo uses a workflow-specific repository secret (`AUTO_CLEANUP_ANTHROPIC_API_KEY`) for this job so auto-cleanup spend stays isolated from terminal-bench, while the runtime environment still uses the standard `ANTHROPIC_API_KEY` name.",
      "",
      "{/* BEGIN AUTO_CLEANUP_WORKFLOW */}",
      "",
      "```yaml",
      "# Periodically cleans up the codebase and maintains a refactor PR.",
      "# SETUP: Add AUTO_CLEANUP_ANTHROPIC_API_KEY, XUM_APP_PRIVATE_KEY, and XUM_APP_ID",
      ...
      '          [ -z "$XUM_APP_ID" ] && echo "Skipping (missing XUM_APP_ID)." && missing=1',
      '          [ -z "$XUM_APP_PRIVATE_KEY" ] && echo "Skipping (missing XUM_APP_PRIVATE_KEY)." && missing=1',
      "",
    ...
    "references/docs/guides/symbol-shortcuts.mdx": [
      ...
      "description: Insert math and trading symbols in the Xum chat input with LaTeX-style backslash commands",
      "---",
      "",
      ...
      "",
      "When a name is a prefix of another command, Xum keeps the menu open instead of guessing. For example `\\in` is a prefix of `\\int` and `\\infty`, so typing `\\in` does **not** convert on its own. Accept it explicitly to disambiguate:",
      "",
    ...
    "references/docs/hooks/tools.mdx": [
      ...
      "",
      "Create `.xum/tool_pre` to validate commands before they run. Exit non-zero to block:",
      "",
      "```bash",
      "#!/usr/bin/env bash",
      "# .xum/tool_pre - runs before every tool",
      "",
      ...
      "",
      "Create `.xum/tool_post` to run validation after tools complete:",
      "",
      "```bash",
      "#!/usr/bin/env bash",
      "# .xum/tool_post - runs after every tool",
      "set -euo pipefail",
      "",
      ...
      "## Set up environment",
      "",
      "Create `.xum/tool_env` to configure your shell environment. This file is **sourced** before every `bash` tool call:",
      "",
      "```bash",
      "# .xum/tool_env - sourced before bash commands",
      "",
      "# direnv",
      ...
      "",
      "| Feature        | `.xum/tool_pre`          | `.xum/tool_post`      | `.xum/tool_env`         |",
      "| -------------- | ------------------------ | --------------------- | ----------------------- |",
    ...
    "references/docs/index.mdx": [
      ...
      "---",
      "",
      "**Xum** makes it easy to run parallel coding agents, each with its own isolated workspace, right from your browser or desktop. Xum is open source and LLM provider-agnostic.",
      "",
      "<video",
      "  autoPlay",
      "  muted",
      ...
      "  playsInline",
      "  controls",
      '  aria-label="Xum product demo video"',
      '  style={{ width: "100%" }}',
      '  src="/img/xum-docs-landing.mp4"',
      "></video>",
      "",
      "## Getting Started",
      "",
      "<Columns cols={2}>",
      '  <Card title="Install Xum" icon="download" href="/install">',
      "    Download and install Xum for your operating system.",
      "  </Card>",
      '  <Card title="Xum Gateway credits" icon="key" href="/getting-started/mux-gateway">',
      "    Log in to Xum Gateway to get evaluation credits (if eligible).",
      "  </Card>",
      "</Columns>",
      "",
      "## What is Xum?",
      "",
      "Xum helps you work with multiple coding agents more effectively via:",
      "",
      ...
      "## License",
      "",
      "Xum is licensed under the [GNU Affero General Public License v3.0 (AGPL-3.0)](https://github.com/coder/mux/blob/main/LICENSE).",
      "",
    ...
    "references/docs/install.mdx": [
      "---",
      "title: Install",
      "description: Download and install Xum for macOS, Linux, and Windows",
      "---",
      "",
      "## Downloads",
      "",
      "### Release Builds",
      "",
      '<Card title="Download Xum" icon="download" href="https://github.com/coder/mux/releases">',
      ...
      "2. Open the DMG file",
      "3. Drag Xum to Applications folder",
      ...
      "2. Make it executable: `chmod +x xum-*.AppImage`",
      "3. Run it: `./xum-*.AppImage`",
      "",
      "#### Windows",
      "",
      "_(alpha)_",
      ...
      "2. Run: `xattr -cr /Applications/xum.app`",
      "3. Run: `codesign --force --deep --sign - /Applications/xum.app`",
      "4. Now you can open the app normally",
      "",
      "## CLI via npm",
      "",
      "The Xum CLI can also be run directly via `npx` without installing the desktop app:",
      "",
      "```bash",
      "# Run agent tasks",
      ...
      "npx @coder/xum server --port 3000",
      "```",
      "",
      "Or install globally:",
      "",
      ...
      "```",
      "",
      "The legacy `mux` package and command forward to the matching Xum release during the transition. See [Mux compatibility](/reference/mux-compatibility) for storage, environment-variable, and downgrade details.",
      "",
    ...
    "references/docs/integrations/vscode-extension.mdx": [
      ...
      "description: Pair Xum workspaces with VS Code and Cursor editors",
      "---",
      "",
      "The Xum VS Code extension allows you to easily pair with Xum during development. Our extension works with VS Code and Cursor.",
      "",
      ...
      "",
      "![Xum VS Code extension screenshot](../img/vscode-ext.webp)",
      "",
      "1. Press `Cmd+Shift+P` (or `Ctrl+Shift+P` on Windows/Linux)",
      '2. Type "xum: Open Workspace"',
      ...
      "</Warning>",
      "",
      "You can find it in the **Secondary Sidebar** under the `xum` container: **Chat (Preview)**.",
      "",
      "1. Open **Chat (Preview)**",
      "2. Select a Xum workspace from the picker",
      "3. Chat normally; use the pencil icon to open the workspace in a new window",
      "",
      "To send messages, Xum must be connected in server/API mode.",
      "",
      "If you hit issues, please report them on the [Xum GitHub issues page](https://github.com/coder/mux/issues).",
      "",
      "## Installation",
      "",
      "### Download",
    ...
    "references/docs/reference/debugging.mdx": [
      ...
      "---",
      "",
      "The **Output** tab streams Xum's backend logs in real time — useful for diagnosing tool failures, MCP server issues, or unexpected behavior.",
      "",
    ...
    "references/docs/reference/storybook.mdx": [
      "---",
      "title: Storybook",
      "description: Develop and test Xum UI states in isolation",
      "---",
      "",
      "Storybook renders Xum's **renderer UI** without running the full Electron app.",
      "",
      "## Starting Storybook",
      "",
      "```bash",
      ...
      "## Writing Stories",
      "",
      "Xum uses two Storybook patterns:",
      "",
    ...
    "references/docs/runtime/devcontainer.mdx": [
      ...
      "## Filesystem layout",
      "",
      "Worktrees live on the host at `~/.xum/src/<project>/<workspace>`. The container mounts this directory (location inside the container depends on your devcontainer.json `workspaceFolder`).",
      "",
      "## Notes",
      "",
    ...
    "references/docs/runtime/docker.mdx": [
      ...
      "## How it works",
      "",
      "1. When you create a Docker workspace, Xum starts a container from your chosen image",
      ...
      "## Using Podman",
      "",
      "When Docker is unavailable, Xum automatically uses Podman as a drop-in container engine. Set `XUM_CONTAINER_CLI` to force a specific container CLI binary instead.",
      "",
      "## Credential sharing",
      "",
      ...
      "- **GitHub CLI** — If `GH_TOKEN` is set in [project secrets](/config/project-secrets), Xum runs `gh auth setup-git` to configure HTTPS authentication (requires `gh` CLI in the container image)",
      "",
    ].join("\n"),
    "references/docs/runtime/index.mdx": [
      "---",
      "title: Runtimes",
      "description: Configure where and how Xum executes agent workspaces",
      "---",
      "",
      "Runtimes determine where and how Xum executes agent workspaces.",
      "",
      ...
      "## Init Hooks",
      "",
      "[Init hooks](/hooks/init) and agent bash tool executions can detect the runtime type via the `XUM_RUNTIME` environment variable:",
      "",
      "- `local` — Local runtime",
    ...
    "references/docs/runtime/local.mdx": [
      ...
      "  Running them simultaneously can cause conflicts. Xum shows a warning when another local workspace",
    ...
    "references/docs/runtime/ssh.mdx": [
      ...
      "---",
      "",
      "Xum can run workspaces on a remote host over SSH. When configured, tool operations (file reads/edits, `bash`, etc.) execute on the remote machine.",
      "",
      "## Threat model",
      "",
      "Xum treats the remote host as potentially hostile. By default it does **not** forward your local keys or credentials.",
      "",
      ...
      "",
      "Xum delegates SSH configuration to your system `ssh` command, so advanced settings live in `~/.ssh/config`.",
      "",
      "Example entry:",
      "",
      "```text",
      ...
      "",
      "<Info>",
      "  Xum delegates to `ssh`. This is an abbreviated reference of common ways `ssh` authenticates.",
      "</Info>",
      "",
      "### Local default keys",
      "",
    ...
    "references/docs/workspaces/compaction/automatic.mdx": [
      ...
      "description: Let Xum automatically compact your conversations based on usage or idle time",
      "---",
      "",
      "Xum can run `/compact` for you to keep context size manageable. There are two types of automatic compaction:",
      "",
      ...
      "2. When you send your next message **at or above the threshold**, Xum runs compaction first, then automatically sends your message as a follow-up",
      "",
      ...
      "",
      "Xum caps the enabled threshold at **90%** to leave a safety buffer before hard context limits.",
      "",
      "### Force-compaction during streaming",
      "",
      "If a single response pushes context usage above your threshold while streaming, Xum may interrupt and compact automatically once you exceed the threshold by an additional buffer (currently **+5%**). After compaction, Xum resumes the conversation automatically.",
      "",
      ...
      "",
      "When enabled for a project, Xum periodically checks for workspaces that:",
      "",
      ...
      "- Xum checks roughly **once per hour**, so compaction may not trigger immediately when the timer expires",
      "",
      "---",
      "",
      "## General notes",
      ...
      "- Auto-compaction requires known model context limits; if Xum can't determine a model's context window, usage-based auto-compaction won't run",
    ...
    "references/docs/workspaces/compaction/index.mdx": [
      ...
      "- [Automatic Compaction](/workspaces/compaction/automatic) — Let Xum compact for you based on usage or idle time",
      "",
    ].join("\n"),
    ...
    "references/docs/workspaces/scratch-chats.mdx": [
      ...
      "---",
      "",
      "Scratch chats let you start working without adding a project. Each chat gets a private folder managed by Xum, so the agent can create and edit files even though the chat is not attached to an existing repository.",
      "",
      ...
      "",
      "Xum creates the chat and its folder when you send the first message. Closing an empty draft does not create a folder.",
      "",
      "## Files and Git",
      "",
      "Scratch chat files are stored under `~/.xum/scratch/<workspace-id>`. The folder persists across app restarts.",
      "",
      "Xum does not initialize a Git repository for a scratch chat. Branch controls, Git status, and code review tools are hidden. You can still use the terminal and other tools that only need a working directory.",
      "",
    ...
    "references/docs/workspaces/xumignore.mdx": [
      "---",
      "title: .xumignore",
      ...
      "",
      "A `.xumignore` file lets you declare which gitignored files should be copied into new worktree workspaces. Existing `.muxignore` files remain readable when `.xumignore` is absent, but new repositories should use the canonical name.",
      "",
      "## Setup",
      "",
      "Create a `.xumignore` file in your project root (alongside `.gitignore`):",
      "",
      "```gitignore",
      ...
      "",
      "> Xum only copies files that are already gitignored in your project and currently exist on disk.",
      "",
      "## Behavior",
      "",
      ...
      "- **Best-effort:** If a file can't be copied, Xum logs debug information and continues — it won't block workspace creation.",
      "",
      "## Where to place it",
      "",
      "Place `.xumignore` in the project root, next to your `.gitignore`. You may want to commit it to your repo so all collaborators share the same sync rules.",
      "",
    ].join("\n"),
    "SKILL.md": [
      "---",
      ...
      "# xum docs",
      "",
      "This built-in skill helps the agent answer questions about **xum** (Coding Agent Multiplexer) without dumping the entire docs into context.",
      "",
      "## How to use",
      "",
      "### Prefer: read the bundled docs snapshot (recommended)",
      "",
      "This skill bundles an **offline snapshot of the xum docs** under `references/docs/`.",
      "",
      "Why prefer the bundled snapshot?",
      "",
      "1. The docs tree below is guaranteed to match what’s embedded.",
      "2. It’s more likely to match _your installed xum version_ (the live site may be ahead).",
      "",
      ...
      "    - Install (`/install`) → `references/docs/install.mdx`: Download and install Xum for macOS, Linux, and Windows",
      "    - **Models**",
      "      - Models (`/config/models`) → `references/docs/config/models.mdx`: Select and configure AI models in Xum",
      ...
      "    - Xum Gateway (`/getting-started/mux-gateway`) → `references/docs/getting-started/mux-gateway.mdx`: Log in to Xum Gateway to get evaluation credits",
      ...
      "    - Server Access (`/config/server-access`) → `references/docs/config/server-access.mdx`: Configure authentication and session controls for xum server/browser mode",
      "    - Vim Mode (`/config/vim-mode`) → `references/docs/config/vim-mode.mdx`: Vim-style editing in the Xum chat input",
      "  - **Guides**",
      "    - GitHub Actions (`/guides/github-actions`) → `references/docs/guides/github-actions.mdx`: Automate your workflows with xum run in GitHub Actions",
      "    - Symbol Shortcuts (`/guides/symbol-shortcuts`) → `references/docs/guides/symbol-shortcuts.mdx`: Insert math and trading symbols in the Xum chat input with LaTeX-style backslash commands",
      "    - Agentic Git Identity (`/config/agentic-git-identity`) → `references/docs/config/agentic-git-identity.mdx`: Configure a separate Git identity for AI-generated commits",
      "    - Prompting Tips (`/agents/prompting-tips`) → `references/docs/agents/prompting-tips.mdx`: Tips and tricks for getting the most out of your AI agents",
      "  - **Integrations**",
      ...
      "",
      "```ts",
      'agent_skill_read_file({ name: "xum-docs", filePath: "references/docs/docs.json" });',
      "```",
      "",
      "2. Read a specific page by path (mirrors `docs/` in the xum repo):",
      "",
